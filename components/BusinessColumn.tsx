...
interface DownloadActions { downloadDepositList: () => void; downloadWorkLog: () => void; }
...
const BusinessColumnContent: React.FC<BusinessColumnProps> = ({ businessId, displayName, portalId, themeColor, bank, sharedSuppliers, onStatusUpdate, onRegisterMasterUpload, onRegisterReset, onRegisterDownloadActions, onWorkstationReset, globalFakeOrderInput, onGlobalFakeMatch, globalUnsentOrderInput, onEdit, onExposeOrderRows, onHasWarnings }) => {
  ...
  const [actions, setActions] = useState<{ downloadDepositList?: () => void; downloadWorkLog?: () => void }>({});

  // 인라인 함수로 넘기면 매 렌더마다 새 참조 → CompanySelector useEffect 무한루프 발생
  // useCallback으로 안정화
  const handleRegisterActions = useCallback((a: { downloadDepositList: () => void; downloadWorkLog: () => void }) => {
    setActions(a);
