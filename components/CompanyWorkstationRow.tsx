                        ...
                        {isFirstSession ? (
                            <>
                                <div className="flex items-center gap-2 flex-wrap">
                                        ...
                                        {(['order', 'deposit', 'invoice'] as const).map((step) => (
                                            <button 
                                                ...
                                                className={`px-1.5 py-0.5 rounded text-[9px] font-black transition-all ${
                                                    workflow[step] 
                                                        ? (step === 'order' ? 'bg-sky-500' : step === 'deposit' ? 'bg-emerald-500' : 'bg-indigo-500') + ' text-white shadow-md'
                                    ...
                                    {deadline && (
                                        <div className="bg-sky-500/10 text-sky-500 px-2 py-0.5 rounded-lg border border-sky-500/30 flex items-center gap-1 shrink-0">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <div className="flex items-center gap-1.5 bg-zinc-950/50 px-2 py-1 rounded-lg border border-zinc-800 shrink-0">
                                            ...
                                            <input 
                                                ...
                                                className="w-20 bg-transparent border-none text-[10px] font-black text-sky-400 placeholder:text-zinc-700 focus:ring-0 p-0 text-right"
                                            />
                                            <button onClick={handleAddAdj} className="text-sky-500 hover:text-white hover:bg-sky-500 rounded p-0.5 transition-all">
                                                <PlusCircleIcon className="w-3 h-3" />
                                            </button>
                                        </div>
                                        <button onClick={onAddSession} className="p-1 bg-zinc-800 text-zinc-500 rounded-lg hover:bg-sky-500 hover:text-white transition-all border border-zinc-700">
                                ...
                                <div className="flex flex-wrap gap-1 items-center">
                                    ...
                                    {isAddingKeyword ? (
                                        <input
                                            ...
                                            placeholder="키워드 입력"
                                            className="text-[9px] bg-zinc-950 text-zinc-300 px-1.5 py-0.5 rounded border border-zinc-700 font-bold w-20 outline-none focus:border-sky-500/50"
                                        />
                                    ) : (
                                        <button onClick={() => setIsAddingKeyword(true)} className="text-[9px] bg-zinc-900/50 text-zinc-600 hover:text-sky-400 px-1.5 py-0.5 rounded border border-dashed border-zinc-800 hover:border-sky-500/30 font-bold transition-colors">+</button>
                                    )}
                                </div>
                            </>
                        ) : (
                                    ...
                                    <div className="mt-1.5 animate-fade-in">
                                        <div className="bg-zinc-950/50 px-2.5 py-1.5 rounded-lg border border-emerald-500/20 relative">
                                            <div className="flex justify-between items-center mb-1">
                                                ...
                                                <button onClick={handleCopyCombined} className={`text-[9px] font-black px-2 py-0.5 rounded border transition-all ${copiedCombinedId ? 'bg-emerald-500 text-white border-emerald-400' : 'bg-zinc-800 text-sky-400 border-zinc-700 hover:text-white'}`}>{copiedCombinedId ? '복사됨!' : '카톡용'}</button>
                                            </div>
                    ...
                    <div className={`flex flex-col items-center ${isFirstSession ? 'gap-2' : 'gap-1'}`}>
                        {localResult ? (
                            <div className="flex flex-col items-center gap-2 animate-fade-in w-full">
                                {isFirstSession && (
                                    <div className="flex items-center justify-center gap-4">
                                        <div className="text-center">
                                            <div className="text-sky-400 font-black text-xl">{companyTotalOrders || Object.values(localResult.summary).reduce((a:any, b:any) => a + b.count, 0)}</div>
                                        ...
                                        {onDownloadMergedOrder ? (
                                            ...
                                            <button onClick={handleDownloadOrder} className="bg-sky-500 text-white hover:bg-sky-600 px-3 py-1 rounded font-black text-[10px] shadow-md flex items-center gap-1.5 transition-all"><ArrowDownTrayIcon className="w-3.5 h-3.5" /><span>받기</span></button>
                                        )}
                                    </div>
                                )}
                                <div className="flex items-center justify-center gap-4">
                                ...
                                <div className="flex items-center gap-2">
                                    <button onClick={() => setShowSummary(!showSummary)} className="text-zinc-600 hover:text-sky-400 text-[9px] font-black uppercase flex items-center gap-1 whitespace-nowrap">{showSummary ? <ChevronUpIcon className="w-3 h-3"/> : <ChevronDownIcon className="w-3 h-3"/>}정산</button>
                                    {excludedList.length > 0 && (
                                        <button onClick={() => setShowExcluded(!showExcluded)} className="text-sky-500 hover:text-sky-400 text-[9px] font-black uppercase flex items-center gap-1 whitespace-nowrap">
                                            제외({excludedList.length})
                                        </button>
                                    )}
                                    <button onClick={resetLocalFile} className="p-1 bg-zinc-900 rounded text-zinc-700 hover:text-sky-500 border border-zinc-800 transition-colors"><ArrowPathIcon className="w-3 h-3" /></button>
                                </div>
                            </div>
                        ) : isLocalProcessing ? (
                            ...
                            <div className="flex flex-col items-center gap-2 animate-fade-in w-full">
                                <div className="flex items-center justify-center gap-4">
                                    <div className="text-center">
                                        {isFirstSession && roundOrderCounts.length > 1 ? (
                                            <>
                                                <div className="text-sky-400 font-black text-xl">{companyTotalOrders}</div>
                                                <div className="text-zinc-600 font-black text-[9px] uppercase tracking-widest">Orders</div>
                                            ...
                                            <>
                                                <div className={`font-black ${isFirstSession ? 'text-sky-400 text-xl' : 'text-indigo-400 text-base'}`}>{isFirstSession ? '' : '+'}{syncedData.orderCount}</div>
                                                <div className="text-zinc-600 font-black text-[9px] uppercase tracking-widest">Orders</div>
                                            </>
                                        )}
                                    </div>
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <button onClick={resetSyncedData} className="p-1 bg-zinc-900 rounded text-zinc-700 hover:text-sky-500 border border-zinc-800 transition-colors"><ArrowPathIcon className="w-3 h-3" /></button>
                                </div>
                            </div>
                        ) : (
                    ...
                    <div className={`flex flex-col items-center ${isFirstSession ? 'gap-2' : 'gap-1'}`}>
                        {!mergeResults ? (
                            ...
                            <div className="flex items-center gap-2 animate-fade-in">
                                ...
                                <button onClick={() => { onVendorFileChange([]); resetMerge(); }} className="p-1 bg-zinc-900 rounded text-zinc-700 hover:text-sky-500 border border-zinc-800 transition-colors shadow-sm"><ArrowPathIcon className="w-3.5 h-3.5" /></button>
                            </div>
                        )}
                    </div>
                </td>
